...
export function auditSite(pages) {
  const html = pages.filter((p) => p.status === 200 && (!p.contentType || /html/i.test(p.contentType)));
  const targetPages = html.length ? html : (pages.filter((p) => p.status === 200).length ? pages.filter((p) => p.status === 200) : pages.slice(0, 10));
  if (!targetPages.length) return null;
  const audits = targetPages.map(auditHeaders);
