...
const psiFile = (id) => `psi-log-${id}.json`;

export async function logPsi(id, entry) {
  const log = await readJson(psiFile(id), []);
  log.push({ at: new Date().toISOString(), ...entry });
  const sliced = log.slice(-400);
  await writeJson(psiFile(id), sliced);
  return sliced.length;
}

export async function psiLog(id) {
  return readJson(psiFile(id), []);
}
