...
export async function probe({ force = false } = {}) {
  ...
  try {
    ...
    const reason = isCloud
      ? 'No drivable Chrome browser found in serverless cloud environment. Vercel serverless functions do not include headless Chrome due to container size limits. Run SEO Workbench locally (`npm start`) with Chrome installed for live browser rendering.'
      : `No drivable Chrome found (tried ${CHANNELS.join(', ')} and a bundled binary). Install Google Chrome, or run: npx playwright install chromium. Details: ${String(e.message).split('\n')[0]}`;
    return (cachedProbe = {
      available: false,
      reason,
    });
