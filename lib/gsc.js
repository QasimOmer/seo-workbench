...
const TOKENS_FILE = process.env.VERCEL ? join(DATA_DIR, '.tokens.json') : join(ROOT, '.tokens.json');
...
export function makeClient() {
  const clientId = process.env.GSC_CLIENT_ID || process.env.GOOGLE_CLIENT_ID;
  const clientSecret = process.env.GSC_CLIENT_SECRET || process.env.GOOGLE_CLIENT_SECRET;
  const redirectUri = process.env.GSC_REDIRECT_URI || process.env.GOOGLE_REDIRECT_URI || `http://localhost:${process.env.PORT || 4321}/api/gsc/callback`;
...
export async function exchangeCode(code) {
  const c = makeClient();
  if (!c) throw new Error('Search Console client is not configured.');
  const { tokens: t } = await c.getToken(code);
  await saveTokens(t);
  return true;
}
