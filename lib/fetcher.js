...
export async function fetchChain(url, opts = {}) {
  const defaultTimeout = process.env.VERCEL ? 5000 : 15000;
  const {
    ua = 'googlebot',
    maxHops = 8,
  ...
  for (let hop = 0; hop <= maxHops; hop++) {
    ...
    try {
      res = await fetch(current, {
        ...
        headers: {
          ...
          'Sec-CH-UA-Mobile': ua === 'googlebot' ? '?1' : '?0',
          'Sec-CH-UA-Platform': ua === 'googlebot' ? '"Android"' : '"Windows"',
    ...
    // WAF / anti-bot fallback: if Googlebot is blocked with 403 or 503 (e.g. Cloudflare DNS verification failure), retry with desktop Chrome
    if ((status === 403 || status === 503) && ua === 'googlebot' && !opts._retriedDesktop) {
      const fallback = await fetchChain(current, { ...opts, ua: 'desktop', _retriedDesktop: true });
...
export async function readSitemap(url, opts = {}, seen = new Set(), depth = 0) {
  const maxDepth = process.env.VERCEL ? 2 : 4;
  if (depth > maxDepth || seen.has(url)) return { urls: [], errors: [] };
  seen.add(url);
  const smTimeout = opts.timeout || (process.env.VERCEL ? 3500 : 8000);
  const res = await fetchChain(url, { ...opts, timeout: smTimeout });
  if (!res.ok || !res.body) {
  ...
  if (isIndex) {
    ...
    for (const b of blocks) {
      ...
      if (loc) locs.push(loc.replace(/<!\[CDATA\[|\]\]>/g, '').trim());
    }
    // On serverless, prioritize key sitemaps (page/post) and parse concurrently
    const prioritized = locs.filter((l) => /page|post/i.test(l));
    const toFetch = (prioritized.length ? prioritized : locs).slice(0, process.env.VERCEL ? 2 : 4);
    const results = await Promise.allSettled(toFetch.map((l) => readSitemap(l, opts, seen, depth + 1)));
    for (const r of results) {
      if (r.status === 'fulfilled' && r.value) {
        urls.push(...r.value.urls);
        if (r.value.errors) errors.push(...r.value.errors);
      }
    }
  } else {
    const maxEntries = process.env.VERCEL ? 150 : 1000;
    for (const b of blocks.slice(0, maxEntries)) {
      const loc = (b.match(/<loc>\s*([\s\S]*?)\s*<\/loc>/i) || [])[1];
      if (!loc) continue;
