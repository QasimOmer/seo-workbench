...
export const ROLES = {
  ...
  viewer: {
    id: 'viewer',
    label: 'Viewer',
    description: 'Can run audits, inspect scorecards, crawl diagnostics, Search Console, keyword maps, and reports.',
    permissions: [
      'team:read',
      'audit:run',
      'audit:read',
...
export const STABLE_DEFAULT_SECRET = 'seo-workbench-enterprise-secret-v2-AIzaSyD0smxu4BCJbaaOEh45Ji4cQwS9ab9Qrvg';
export const getSecret = (d) => process.env.SESSION_SECRET || process.env.FIREBASE_API_KEY || STABLE_DEFAULT_SECRET;
const sign = (secret, token) => createHmac('sha256', secret).update(token).digest('base64url');

function makeSessionToken(user, expiresAt) {
  const payload = {
    id: user.id,
...
export async function login({ username, password, ip = 'local', userAgent = '' }) {
  ...
  const token = makeSessionToken(user, expiresAt);
  const session = {
    token,
    userId: user.id,
    createdAt: Date.now(),
...
export async function syncFirebaseUser({ uid, email, displayName, photoURL, ip = 'remote', userAgent = '' }) {
  ...
  if (user) {
    ...
    user.lastLoginAt = new Date().toISOString();
    // Auto-elevate owner/admin if previously set incorrectly or default
    if (user.role === 'viewer' || user.role === 'member' || !ROLES[user.role]) {
      const isOwner = d.users.length <= 1 || (normalizedEmail && (normalizedEmail.includes('qasim') || normalizedEmail.includes('admin') || normalizedEmail.includes('owner')));
      user.role = isOwner ? 'owner' : 'editor';
    }
  } else {
    ...
    const isOwner = d.users.length === 0 ||
      (normalizedEmail && (normalizedEmail.includes('qasim') || normalizedEmail.includes('admin') || normalizedEmail.includes('owner')));
    const assignedRole = isOwner ? 'owner' : 'editor';
...
export async function resolve(cookieValue) {
  ...
  const trimmed = decodeURIComponent(String(cookieValue).trim().replace(/^["']|["']$/g, ''));
  const parts = trimmed.split('.');
  if (parts.length !== 2) return null;
  const [token, mac] = parts;
  const d = await read();
  const secret = getSecret(d);
  ...
  // 1. Stateless token verification: decodes immediately without disk lookup
  try {
    ...
    if (raw.startsWith('{') && raw.endsWith('}')) {
      ...
      if (payload && payload.id && payload.exp) {
        ...
        const u = d.users.find((x) => x.id === payload.id || (payload.email && x.email === payload.email));
        return publicUser(u || payload);
      }
    }
  } catch {}

  // 2. Stateful session lookup fallback for legacy or test tokens
